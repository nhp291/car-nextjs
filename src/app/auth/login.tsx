...
export default function Login() {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    if (!user) {
      ...
      // Lưu user vào localStorage hoặc context nếu muốn
    }
  };

  return (
    <main className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 via-white to-indigo-100">
      <form onSubmit={handleLogin} className="bg-white/80 backdrop-blur-lg border border-blue-100 rounded-2xl shadow-2xl p-8 w-full max-w-md flex flex-col gap-6">
        <h1 className="text-3xl font-extrabold text-indigo-900 mb-2 text-center">Đăng nhập</h1>
        ...
        <button type="submit" className="bg-blue-600 text-white font-bold rounded-full px-8 py-2 mt-2 hover:bg-blue-700 transition">Đăng nhập</button>
      </form>
    </main>
  );
} 